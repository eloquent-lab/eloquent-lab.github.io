...
def run(model, tokenizer, contexts, questions, args, batchsize=10, verbose=False, answerability=False):
    ...
    for input_text in questions:
        ...
        for text in contexts:
            ...
            r1s = np.arange(10) if True else np.concatenate([np.arange(10), np.arange(10)])
...
def shuffle(questions, frac):
    r = ["" for x in questions]
    ord = np.arange(len(questions))
    np.random.shuffle(ord)
    for x in range(ord.shape[0] // 2):
        q1 = questions[ord[x]][0].split()
        q2 = questions[ord[x + 1]][0].split()
        fq1 = int(frac * len(q1))
        fq2 = int(frac * len(q2))
        q1 = q1[:fq1] + q2[fq2:]
        q2 = q2[:fq2] + q1[fq1:]
        r[ord[x]] = [" ".join(q1)] + questions[ord[x]][1:]
        r[ord[x + 1]] = [" ".join(q2)] + questions[ord[x + 1]][1:]
    return r
...
def results(similiarity, prew, preq, args, questionset, windows, minsize, maxsize, use_answers, global_args):
    r = []
    questionsetpre = preq([[y[0] if use_answers else " ".join(y) for y in x[1]] for x in questionset])
    ...
    for arg in args:
        ...
        for (name, questionstext), questions in zip(questionset, questionsetpre):
            ...
            with open(("outs/" + name + "-" + arg).replace(".", "-") + "best_texts.txt", "a") as f2:
                ...
                numtop = 15
                topquestions = [sorted(list(zip(questionstext, x)), key=lambda x: x[1], reverse=True)[:numtop] for x
                                in sct.tolist()]
                avg_complexity = 0
                avg_answerability = 0
...
def evaluate_one(text, questions_eval, questions_gold, global_args):
    ...
    windows1 = [(".".join(x),) for x in windows]
    ...
    ns2 = [x[:3] for x in questionslist]
    ...
    r = results(cosinesimilarity, nnsim.nnembed, nnsim.nnembed, args, questionslist,
                windows1 if True else windows, minsize, maxsize, False, global_args)
    return r
